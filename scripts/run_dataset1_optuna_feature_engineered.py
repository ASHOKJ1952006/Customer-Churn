"""
run_dataset1_optuna_feature_engineered.py — Combines Optuna tuning with feature engineering
for potentially the best accuracy. Uses 4-model ensemble (RF, XGBoost, LightGBM, CatBoost)
with engineered features. Saves under 'ds1ofe_' prefix.
"""
...
from pipeline import (
    ...
    build_final_models_from_optuna, tune_threshold, evaluate,
...
# 1. Average monthly spend and charge increase
df['AvgMonthlySpend'] = df['TotalCharges'] / (df['tenure'] + 1)
df['ChargeIncrease'] = df['MonthlyCharges'] - df['AvgMonthlySpend']

# 2. Service engagement count
...
print("Engineered features added: ['AvgMonthlySpend', 'ChargeIncrease', 'NumServices', 'TenureGroup', 'HighRiskCombo', 'IsNewCustomer']")
...
# =========================================================
# Run Optuna-tuned 4-model pipeline with feature engineering
# =========================================================
print("\nTuning and training with feature-engineered data using Optuna...")
...
rf_study = tune_random_forest_optuna(X_train, y_train, n_trials=50)
print("Best RF CV F1:", rf_study.best_value)

print("\nTuning XGBoost (Optuna)...")
xgb_study = tune_xgboost_optuna(X_train, y_train, n_trials=50)
print("Best XGBoost CV F1:", xgb_study.best_value)

print("\nTuning LightGBM (Optuna)...")
lgbm_study = tune_lightgbm_optuna(X_train, y_train, n_trials=50)
print("Best LightGBM CV F1:", lgbm_study.best_value)

print("\nTuning CatBoost (Optuna)...")
cat_study = tune_catboost_optuna(X_train, y_train, n_trials=50)
...
print("\nTraining stacked ensemble (4 models with feature engineering)...")
...
print("Final metrics (Optuna + Feature Engineering):", final_metrics)
...
save_model_files(result, prefix='ds1ofe')

joblib.dump(final_metrics, 'ds1ofe_final_metrics.pkl')
joblib.dump(overfit_df, 'ds1ofe_overfit_df.pkl')
joblib.dump(eff_df, 'ds1ofe_efficiency_df.pkl')

print("\n\nOptuna + Feature Engineering pipeline complete.")
print("This should combine the best of both approaches for potentially higher accuracy.")
